...
export class UserBorrowbookComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._bookService.debugHash(this.thisUrl);
  ...
  borrowBookForm: FormGroup = new FormGroup({
    ...
    returnBookDate: new FormGroup({
      "value": new FormControl("", [Validators.required, this.checkReturnBookDateValid.bind(this)]),
      "type": new FormControl(""),
      "meaningLibrarian": new FormControl(""),
      "meaningOther": new FormControl(""),
    }),
...
// Передаем выбранный из таблицы объект.
...
// Функции для switch
  touchSwitch(stateSwitch: BorrowBook) {
...
// Функции для заполнения формы и подготовки к отправке в БД
